...
1. 使用虚函数（Virtual Function）
	1. 我们在基类中定义一个虚函数，这个函数可以在任何派生类中被“重写”或“定制”。
	2. 使用关键字 `virtual` 来声明。
2. 创建派生类并重写虚函数
	1. 在派生类中，我们提供该虚函数的具体实现。
3. 通过基类的引用或指针调用虚函数
	1. 当我们使用基类定义的指针或引用来调用虚函数时，实际调用的是对象的实际类型（派生类）中的函数版本。

**注意**
1. **基类虚函数未实现导致虚表缺失**
	1. 基类 `RemoteControl` 中的 `virtual void openUtils()` 虽然声明为虚函数，但**未提供具体实现且未声明为纯虚函数**。这会引发以下问题：
		-   **虚函数表（vtable）不完整**：编译器需要为包含虚函数的类生成虚表，而虚表中必须包含所有虚函数的地址。若基类未实现该虚函数，虚表中对应位置无法填充有效地址，导致链接阶段报错 `undefined reference to vtable`
		- ​**链接器无法找到基类函数实体**：即使派生类实现了该函数，基类本身的虚函数仍需有实现（或明确声明为纯虚函数），否则链接器会因找不到基类函数实体而报错。
	2.  **解决思路**
		- 将基类的虚函数声明为纯虚函数（ `virtual void openUtils() = 0;` ），使其成为抽象基类。
			- **基类无法实例化**。
			- **强制派生类实现**：派生类必须实现纯虚函数，否则自身也会成为抽象类。
		- 实现基类虚函数。
...
2. **不能直接实例化**。
	1. 抽象类是不完整的。
3. **用于提供基础结构**。
	1. 抽象类的主要目的是为派生类提供一个共同的基础结构，确保所有派生类都有一致的接口和行为。

> 1. **多态基类的析构函数应始终声明为 `public virtual` （除非有特殊设计需求）**。
> 2. 若基类不允许直接实例化，可将其析构函数声明为 `protected`，但需通过公共接口管理对象销毁。
...
一个类作为接口通过以下步骤实现：
1. **定义抽象类**：创建一个包含纯虚函数的抽象类，这些函数构成了接口的一部分。这些函数在抽象类中只有声明没有具体实现。
2. **派生类实现接口**：派生类继承抽象类，并实现其中的纯虚函数，以具体实现接口定义的方法。

> **接口更多的在动作！**，接口的重要性要高于实现细节。

## 2.10 模板

泛型编程，让**类或函数能够适用于多种不同的数据类型**从而不需要重复编写相似地代码。
